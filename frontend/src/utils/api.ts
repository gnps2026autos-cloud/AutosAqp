...
const API_BASE_URL =
  process.env.EXPO_PUBLIC_BACKEND_URL || 'https://autosaqp-backend.onrender.com';
...
export const authAPI = {
  register: async (data: RegisterPayload): Promise<any> => {
  ...
  updateProfile: async (
    data: { name?: string; phone?: string; picture?: string },
    token?: string
  ): Promise<User> => {
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
